...
class voletsEtOuvrants extends eqLogic {
	...
	public static function deamon_info() {
		...
		foreach(eqLogic::byType('voletsEtOuvrants') as $Volet){
			if($Volet->getIsEnable()){
				...
				if($Volet->getConfiguration('EventMvtCmd') != '' ){				
					$listener = listener::byClassAndFunction('voletsEtOuvrants', 'EventRoller', array('Volets_id' => $Volet->getId()));
	...
	public function StopListener() {
		...
		$listener = listener::byClassAndFunction('voletsEtOuvrants', 'EventRoller', array('Volets_id' => $this->getId()));
		if (is_object($listener)) {
			$listener->remove();
		}
	}
	...
	public function StartListener() {
		if($this->getIsEnable()){
			$listener = listener::byClassAndFunction('voletsEtOuvrants', 'EventRoller', array('Volets_id' => $this->getId()));
			$EventMvtCmd=$this->getConfiguration('EventMvtCmd');
			if ($EventMvtCmd != ''){
              	$cmdEventMvt=cmd::byId(str_replace('#','',$this->getConfiguration('EventMvtCmd')));
              
              	if (is_object($cmdEventMvt)) {
                  
                  $this->writeLog('info','StartListener pour la cmd : ' . $cmdEventMvt->getHumanName() .'(id:'.  $this->getConfiguration('EventMvtCmd').')',$this->getHumanName());
                  if (!is_object($listener)) {
                      $listener = new listener();
                  }
                  $listener->setClass('voletsEtOuvrants');
                  $listener->setFunction('EventRoller');
                  $listener->setOption(array('Volets_id' => $this->getId()));
                  $listener->emptyEvent();	
                  $listener->addEvent($EventMvtCmd);
                  $listener->save();			
                }
	...
	/* functions listener */
	public static function EventRoller($_option) {
		...
		if (is_object($Volet) && $Volet->getIsEnable()) {
			...
			if ($_option['value'] == $Volet->getConfiguration('cfgStatutUp')) {
				$Volet->writeLog('info','		=> action up externe',$Volet->getHumanName());
				cache::set('voletsEtOuvrants::ChangeStateStart::'.$Volet->getId(),microtime(true), 0);
				cache::set('voletsEtOuvrants::Move::'.$Volet->getId(),true, 0);
				cache::set('voletsEtOuvrants::ChangeState::'.$Volet->getId(),true, 0);
				cache::set('voletsEtOuvrants::ChangeWay::'.$Volet->getId(),"up", 0);
				cache::set('voletsEtOuvrants::stopAction::'.$Volet->getId(),false, 0);
				$Volet->UpdateRollerState();
			} elseif ($_option['value'] == $Volet->getConfiguration('cfgStatutDown')) {
				$Volet->writeLog('info','		=> action up down',$Volet->getHumanName());	
				cache::set('voletsEtOuvrants::ChangeStateStart::'.$Volet->getId(),microtime(true), 0);
				cache::set('voletsEtOuvrants::Move::'.$Volet->getId(),true, 0);
				cache::set('voletsEtOuvrants::ChangeState::'.$Volet->getId(),false, 0);
				cache::set('voletsEtOuvrants::ChangeWay::'.$Volet->getId(),"down", 0);
				cache::set('voletsEtOuvrants::stopAction::'.$Volet->getId(),false, 0);
			} elseif ($_option['value'] == $Volet->getConfiguration('cfgStatutStop')) {
				$Volet->writeLog('info','		=> action stop externe',$Volet->getHumanName());
				cache::set('voletsEtOuvrants::stopAction::'.$Volet->getId(),true, 0);
				if($Volet->getConfiguration('cfgCmdStop') != ''){
					$cmd=cmd::byId(str_replace('#','',$Volet->getConfiguration('cfgCmdStop')));
					if(is_object($cmd)){
						$Volet->writeLog('info',' - Execution de la commande '.$cmd->getHumanName(),$Volet->getHumanName());	
						$cmd->execute(null);
					} else {
						throw new Exception($Volet->getHumanName().' la commande ' . $Volet->getConfiguration('cfgCmdStop') . ' n\'exitse pas');
					}
					
				}else{
					if(cache::byKey('voletsEtOuvrants::Move::'.$Volet->getId())->getValue(false)){
						if(cache::byKey('voletsEtOuvrants::ChangeState::'.$Volet->getId())->getValue(false)) {
							cache::set('voletsEtOuvrants::ChangeWay::'.$Volet->getId(),"up", 0);
							$cmdId=str_replace('#','',$Volet->getConfiguration('cfgCmdUp'));
						} else {
							cache::set('voletsEtOuvrants::ChangeWay::'.$Volet->getId(),"down", 0);
							$cmdId=str_replace('#','',$Volet->getConfiguration('cfgCmdDown'));
						}
						$cmd=cmd::byId($cmdId);
						if(is_object($cmd)){
							$Volet->writeLog('info',' Execution de la commande '.$cmd->getHumanName(),$Volet->getHumanName());	
							...
							throw new Exception($Volet->getHumanName().' la commande ' . $cmdId . ' n\'exitse pas');
						}
					}
				}		
				
				if(cache::byKey('voletsEtOuvrants::Move::'.$Volet->getId())->getValue(false)){
					$Volet->writeLog('info',' Mise a jours manuel de la hauteur',$Volet->getHumanName());	
					cache::set('voletsEtOuvrants::ChangeStateStop::'.$Volet->getId(),microtime(true), 0);
					$Volet->UpdateRollerState('');
				}
	...
	public function UpdateRollerState($rollerStateValue) {
		...
		if($this->getConfiguration('cfgPluginManageRollerState') != '' && $this->getConfiguration('cfgPluginManageRollerState') == '1' ){
			...
			$this->writeLog('info', '			=> Function UpdateRollerState --> rollerState not manage by plugin',$this->getHumanName());		
		}
	}
